...
for (let i = 0; i < strAlf.length; i++) {
    arrCodes[i] = strAlf.charCodeAt(i);
}

for (let i = 0; i < strNum.length; i++) {
    arrNums[i] = strNum.charCodeAt(i);
}

for (let i = 0; i < optionalExpensesInputs.length; i++) {
    optionalExpensesInputs[i].addEventListener('input', function() {
        let value = optionalExpensesInputs[i].value;

        for (let j = 0; j < value.length; j++) {
            if (!~arrCodes.indexOf(value[j].charCodeAt())) {
                optionalExpensesValue.textContent = 'Введите на русском';
                optionalExpensesInputs[i].value = value.slice(0, j);
            } else {
...
for (let i = 1; i < expensesInputs.length; i = i + 2) {
    expensesInputs[i].addEventListener('input', function() {
        let value = expensesInputs[i].value;
